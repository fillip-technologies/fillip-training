...
export default function EnrollFormModal({ open, onClose, course }) {
  ...
  const validators = {
    ...
    course: (v) => (!v.trim() ? "Course is required" : ""),
  ...
  const handleChange = (e) => {
    ...
    if (name === "phone") processed = processed.replace(/\D/g, "").slice(0, 10);
  ...
  const handleSubmit = async (e) => {
    ...
    const touchAll = Object.keys(form).reduce(
      (a, k) => ({ ...a, [k]: true }),
      {}
    );
    setTouched(touchAll);
                  ...
                  <div key={f.name} className="space-y-1">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        onBlur={handleBlur}
                        className={
                          errors[f.name] ? "border-red-500 bg-red-50" : ""
                        }
                      />
                      {errors[f.name] && (
